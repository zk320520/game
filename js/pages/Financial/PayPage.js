        ...
        requestApi.get('user/getUserInfo', {
            ...
            this.setState({
                ...
                requestApi.get('system/getCurrencyRate', null, null, res => {
                    ...
                    } else if (result.CountryCode == 886) {
                        this.setState({
    ...
    _handlePay(type) {
        ...
        if (type == 'chaoshang') {
            requestApi.postJson('wallet/cvsdeposit', {
                ...
                Portal.remove(key);
                console.log(result);
                NavigationUtil.goPage({ uri: result.paymentUrl }, 'FinancialPayment');
            });
        } else {
                ...
                <SafeAreaView style={{ flex: 1, }}>
                    ...
                    <View style={{ marginTop: 10, marginHorizontal: 20, display: 'flex', flexDirection: 'row', }}>
                        <View style={{ flex: 1, dispatch: 'flex', alignItems: 'flex-start', }}>
                            ...
                            <Text style={{ marginTop: 10, color: '#AC7508', fontSize: 12, fontWeight: 'bold' }}>生技優質組：</Text>
                            <Text style={{ marginTop: 10, color: '#AC7508', fontSize: 12, fontWeight: 'bold' }}>扭力肽x1、胜肽牙膏x2、鳳梨濃縮原汁x2、美嚴肅膠原蛋白x1</Text>
                        </View>
                        <View style={{ flex: 1, dispatch: 'flex', alignItems: 'flex-end', }}>
                            ...
                            <Text style={{ marginTop: 10, color: '#AC7508', fontSize: 12, fontWeight: 'bold' }}>2019.07.10 13:00:00</Text>
                            <Text style={{ marginTop: 10, color: '#895D05', fontSize: 14, fontWeight: 'bold' }}>訂單金額</Text>
                            <Text style={{ marginTop: 10, color: '#AC7508', fontSize: 12, fontWeight: 'bold' }}>${this.state.Account}  {this.state.CountryCode == '86' ? '台幣' : this.state.CountryCode == '886' ? '人民幣' : '美金'}</Text>
                        </View>
                    </View>
                    <View style={{ marginTop: 10, alignItems: 'center', marginHorizontal: 20 }}>

                        {
                            this.state.CountryCode == 86 ?
                                <Fragment>
