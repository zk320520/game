...
class StoreValuePage extends Component {
    ...
    componentDidMount() {
        ...
        requestApi.get('wallet/getDepositRecord', {
            ...
            Portal.remove(key);
            let deespoistRecord = result.deespoistRecord.filter(item => item.Uid == this.props.userInfo.Uid);
            deespoistRecord.forEach(item => {
                item.PayStatus = item.Status[Object.keys(item.Status)[Object.keys(item.Status).length - 1]] == 'HASPAYED'
            });
            console.log(deespoistRecord);
            this.setState({
    ...
    _handleLink() {
        NavigationUtil.goPage({ amount: this.state.CurrentValue }, 'FinancialPay');
    }
